...
class Persona
  ...
  def name
    "#{api.get(personal, 'personal_fname')} #{api.get(personal, 'personal_faname')}"
  end

  def postcode
    api.get(home, 'home_postcode')
  end

  def property_type
    api.get(home, 'home_property_type')
  end

  def energy_bill
    bills = api.transaction_data_cache(@persona_id)["field_ds_utility_billing"]
    ...
    bills[0..11].reduce(0) { |acc, bill| acc += bill['field_utility_billing_total'].to_f }.round(2)
  end
  ...
  def home
    api.pds_data_cache(@persona_id)['field_ds_home'].values.first
  end

  def personal
    api.pds_data_cache(@persona_id)['field_ds_personal_details'].values.first
  end
